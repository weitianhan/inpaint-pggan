...
class Encoder(nn.Module):
    def __init__(self,
        ...
        nin = []
        nin = D_conv(nin, self.num_channels, oc, conv_kernel_size, 1, padding_size, act, iact, negative_slope, True,
                    self.use_wscale, self.use_gdrop, self.use_layernorm, gdrop_param)
        nins.append(nin)
        for I in range(R-1, 3, -1):
            ...
            lods.append(nn.Sequential(*net))
            nin = []
            nin = D_conv(nin, self.num_channels, oc//2, conv_kernel_size, 1, padding_size, act, iact, negative_slope, True,
                        self.use_wscale, self.use_gdrop, self.use_layernorm, gdrop_param)
            nins.append(nin)
